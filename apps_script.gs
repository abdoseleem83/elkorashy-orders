...
// ===== مراحل الطلب التلاتة (لازم تطابق نفس النصوص في index.html بالظبط) =====
// ⚠️ المراحل بقت اتنين بس: تم استلام الطلب ← تم تنفيذ الطلب.
// STATUS_PENDING سايبينه بنفس الاسم عشان باقي الكود، بس قيمته بقت
// 'تم استلام الطلب' — فأي طلب جديد بيتسجّل بالمرحلة الأولى الجديدة.
const STATUS_RECEIVED = 'تم استلام الطلب';
const STATUS_PENDING = STATUS_RECEIVED;
...
function doPostInner_(e) {
  ...
  // ===== تعديل/إضافة صنف من تبويب "إدارة" — بتوكن الإدارة، مش توكن الأدمن =====
  if (data.action === 'manageUpdateCatalogItem') {
    if (!isValidManageToken_(data.token)) return denyManage_();
    const ok = updateCatalogItem_(data.item || {});
    return ContentService.createTextOutput(JSON.stringify({ ok: ok }))
      .setMimeType(ContentService.MimeType.JSON);
  }
  if (data.action === 'manageAddCatalogItem') {
    if (!isValidManageToken_(data.token)) return denyManage_();
    const ok = addCatalogItem_(data.item || {});
    return ContentService.createTextOutput(JSON.stringify({ ok: ok }))
      .setMimeType(ContentService.MimeType.JSON);
  }

  // رفع ملف إكسيل جماعي — من متابعة المخزن (توكن الأدمن) أو تبويب الإدارة
  // (توكنه هو). الاتنين بينادوا على نفس المنطق (bulkUpdateCatalog_).
  if (data.action === 'uploadCatalog') {
    if (!isValidAdminToken_(data.token)) return denyAdmin_();
    const result = bulkUpdateCatalog_(data.rows || [], data.mode || 'full');
    return ContentService.createTextOutput(JSON.stringify(Object.assign({ ok: true }, result)))
      .setMimeType(ContentService.MimeType.JSON);
  }
  if (data.action === 'manageUploadCatalog') {
    if (!isValidManageToken_(data.token)) return denyManage_();
    const result = bulkUpdateCatalog_(data.rows || [], data.mode || 'full');
    return ContentService.createTextOutput(JSON.stringify(Object.assign({ ok: true }, result)))
      .setMimeType(ContentService.MimeType.JSON);
  ...
  if (data.action === 'update') {
    // ✏️ التعديل الجديد: مابنحدّثش الصف القديم — بنعلّمه "مُعدّل" (يفضل ظاهر
    // للأدمن كنسخة قديمة)، وبنضيف الطلب الجديد كصف جديد برقم وتاريخ جديد.
    const rows = sheet.getDataRange().getValues();
    const oldId = data.oldId != null ? String(data.oldId) : '';
    if (oldId) {
      for (let i = 1; i < rows.length; i++) {
        if (String(rows[i][0]) === oldId) {
          sheet.getRange(i + 1, 8).setValue(STATUS_EDITED);   // عمود الحالة
          break;
        }
      }
    }
    // منع التكرار: لو الطلب الجديد (data.id) اتسجّل قبل كده، نرجّع رقمه
    for (let i = 1; i < rows.length; i++) {
      if (String(rows[i][0]) === String(data.id)) {
        return ContentService.createTextOutput(JSON.stringify({ ok: true, orderNo: rows[i][10] || '', duplicate: true }))
          .setMimeType(ContentService.MimeType.JSON);
      }
    }
    ...
    sheet.appendRow([data.id, new Date(data.ts), data.distName, data.distRegion || '', String(data.distPhone || ''), data.note || '', itemsSummary, STATUS_PENDING, JSON.stringify(data.items), data.warehouse || '', orderNo]);
    ...
    // 🔒 idempotent: لو نفس رقم الطلب (data.id) اتسجّل قبل كده — سواء المستخدم
    // ضغط إرسال مرتين، أو النت أعاد المحاولة — مانضيفش صف تاني، بنرجّع رقم الطلب
    // الموجود زي ما هو. ده بيمنع ظهور الطلب مرتين في متابعة المخزن.
    const existRows = sheet.getDataRange().getValues();
    for (let i = 1; i < existRows.length; i++) {
      if (String(existRows[i][0]) === String(data.id)) {
        return ContentService.createTextOutput(JSON.stringify({ ok: true, orderNo: existRows[i][10] || '', duplicate: true }))
          .setMimeType(ContentService.MimeType.JSON);
      }
    }
    const orderNo = getNextOrderNumber_(data.warehouse);
    ...
    sheet.appendRow([
      ...
      data.note || '', itemsSummary, data.status || STATUS_PENDING, JSON.stringify(data.items),
...
// الأدمن يوافق من داخل التطبيق (تبويب "متابعة المخزن") قبل ما يشتغل.
...
function handleRequestPasswordReset(data) {
  ...
  msg += 'افتح تطبيق الإدارة → متابعة المخزن → طلبات الباسورد عشان توافق أو ترفض.';
  queueWhatsApp_(msg);
...
// ⚠️ شيت طلبات إعادة التعيين نفسه مالوش عمود "اسم الموزع" (بس اسم المستخدم
// وكلمة السر الجديدة)، فكنا بنعرض للأدمن اسم المستخدم بس — وده مش واضح
// كفاية (اسم المستخدم ممكن يكون مختلف تمامًا عن اسم الموزّع الحقيقي).
// دلوقتي بندوّر على اسم الموزّع المرتبط باسم المستخدم ده من شيت المستخدمين
// ونرجّعه مع الطلب، عشان الأدمن يعرف يوافق على طلب مين بالظبط.
...
// لسه ما اتنفذتش بالكامل (يعني حالتها "تحت التنفيذ" أو "تم استلام الطلب")
// **ومخزنها طنطا بس**. أي طلب من مخزن تاني (الاسكندرية، المنصورة اكسا، العاشر
// اكسا) مبيتحجزش خالص. أول ما الطلب يتنفذ بالكامل أو يتحذف، الحجز بيروح لوحده —
// ولو فيه طلبين لسه تحت التنفيذ لنفس الصنف واتنفذ واحد بس، بيفضل محجوز قيمة
// التاني بس (كل طلب بيتحسب لوحده حسب حالته).
...
//   بضاعة محجوزة = مجموع كميات الطلبات اللي لسه ماتنفذتش بالكامل لكل صنف (بالعود/القطعة)
...
function buildReservedMap_(csheet) {
  ...
  try {
    ...
    orows.forEach(r => {
      if (!r[0]) return;
      if (String(r[7] || '').trim() === STATUS_DONE) return; // اتنفذ بالكامل، مش محجوز
...
// يعني أي حد يفتح كود الصفحة كان يشوف الرقم ويدخل على "متابعة المخزن".
...
// ===== تبويب "إدارة" الجديد — PIN منفصل تمامًا عن الأدمن (3184) =====
// بنفس فلسفة PIN الأدمن بالظبط: الرقم متخزن في Script Properties (مش في
// كود العميل العام)، وكل نداء إداري لازم توكن صالح 12 ساعة.
//
// شغّل الدالة دي مرة واحدة من محرر Apps Script عشان تحدد رقم تبويب الإدارة.
function setManagePin() {
  PropertiesService.getScriptProperties().setProperty('MANAGE_PIN', '311984');
}
function getManagePin_() {
  const props = PropertiesService.getScriptProperties();
  let pin = props.getProperty('MANAGE_PIN');
  // ⚠️ أول استدعاء، لو الرقم لسه ماتظبطش، بنحطه تلقائي — عشان مايحتاجش
  // حد يفتح محرر Apps Script ويشغّل دالة يدويًا. لو عايز تغيّره بعد كده،
  // شغّل setManagePin() برقم جديد جوه القوسين وبس.
  if (!pin) { pin = '311984'; props.setProperty('MANAGE_PIN', pin); }
  return pin;
}
function manageLogin_(pin) {
  const real = getManagePin_();
  if (!real) return { ok: false, reason: 'not_configured' };
  if (String(pin || '').trim() !== real) return { ok: false, reason: 'wrong_pin' };
  const token = Utilities.getUuid();
  const expiry = Date.now() + 12 * 60 * 60 * 1000; // 12 ساعة
  const props = PropertiesService.getScriptProperties();
  props.setProperty('MTOKEN_' + token, String(expiry));
  return { ok: true, token: token };
}
function isValidManageToken_(token) {
  if (!token) return false;
  const props = PropertiesService.getScriptProperties();
  const exp = props.getProperty('MTOKEN_' + token);
  if (exp) {
    if (Date.now() > Number(exp)) { props.deleteProperty('MTOKEN_' + token); return false; }
    return true;
  }
  // ⚠️ الأدمن ليه نفس صلاحية الإدارة (أو أعلى) — فبنقبل توكن الأدمن هنا كمان.
  // ده بيخلي زرار "إدارة الأصناف" اللي جوه شاشة الإدارة يشتغل على طول من غير
  // ما نطلب من الأدمن رقم سري تاني، ومن غير ما نحط أي رقم في كود العميل العام.
  return isValidAdminToken_(token);
}
function denyManage_() {
  return ContentService.createTextOutput(JSON.stringify({ ok: false, error: 'unauthorized' }))
    .setMimeType(ContentService.MimeType.JSON);
}

// ---- طلبات تسجيل حسابات جديدة (كانت بس بتوصل واتساب، دلوقتي قايمة حقيقية) ----
// شيت المستخدمين: A=username, B=password, C=distName, D=distRegion, E=distPhone,
// F=status, G=date, H=distType. "قيد المراجعة" = لسه محتاج موافقة.
...
// موافقة: الحالة بتتغيّر لـ"مفعل" ويقدر يدخل بحسابه على طول.
// رفض: بتتغيّر لـ"مرفوض" — الصف يفضل موجود للمرجع (مفيش حذف نهائي).
...
// ---- تعديل/إضافة أصناف من جوه التطبيق (بدل فتح الشيت يدويًا) ----
...
function addCatalogItem_(data) {
  ...
  const rows = sheet.getDataRange().getValues();
  for (let i = 1; i < rows.length; i++) {
    if (String(rows[i][0]).trim() === name) return false;
  }
  sheet.appendRow([
...
// ⚠️ الرفع الجماعي (استيراد إكسيل) — الزرار ده كان موجود من زمان في الواجهة
// ("رفع الأصناف/الرصيد" في متابعة المخزن)، بس السيرفر مكانش عنده الكود اللي
// يستقبله أصلاً (action 'uploadCatalog' مكانتش متعرّفة هنا خالص) — يعني
// الزرار ده كان بيفشل بصمت كل مرة. دلوقتي اتصلح، وبقى متاح من مكانين:
// متابعة المخزن (بتوكن الأدمن) وتبويب الإدارة الجديد (بتوكن الإدارة).
//
// mode='full': بيحدّث كل الأعمدة لصف موجود، وبيضيف صف جديد لو الاسم مش موجود.
// mode='stock': بيحدّث عمود الرصيد بس، وبيسجّل الأسامي اللي مش موجودة كـ"notFound".
function bulkUpdateCatalog_(rows, mode) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (n && !(n in nameToRow)) nameToRow[n] = i + 1; // رقم الصف الفعلي (1-indexed)
  }
  ...
  (rows || []).forEach(row => {
    ...
    if (r) {
      if (mode === 'stock') {
        if (row.stock !== undefined) sheet.getRange(r, 8).setValue(row.stock === '' ? '' : Number(row.stock));
      } else {
        sheet.getRange(r, 2).setValue(row.main || '');
        sheet.getRange(r, 3).setValue(row.sub || '');
        sheet.getRange(r, 4).setValue(Number(row.unit) || 1);
        sheet.getRange(r, 5).setValue(row.product || name);
        sheet.getRange(r, 6).setValue(row.color || '');
        sheet.getRange(r, 7).setValue(row.price === '' || row.price == null ? '' : Number(row.price));
        sheet.getRange(r, 8).setValue(row.stock === '' || row.stock == null ? '' : Number(row.stock));
        sheet.getRange(r, 9).setValue(row.disabled ? 'لا' : '');
      }
      updated++;
    } else if (mode === 'full') {
      sheet.appendRow([
        name, row.main || '', row.sub || '', Number(row.unit) || 1, row.product || name, row.color || '',
      ...
      notFound.push(name); // وضع "الرصيد بس" ومفيش صنف بالاسم ده — نبلّغ عنه بدل ما نتجاهله
    }
  });
  return { updated: updated, added: added, notFound: notFound };
}

// بيبني ملف إكسيل فيه كل الأصناف — تصدير كامل عشان تقدر تراجع/تعدّل أوفلاين
// وترفعه تاني بنفس الشكل اللي "رفع الأصناف" بيتوقعه بالظبط.
...
function doGet(e) {
  ...
  // تسجيل دخول تبويب "إدارة" الجديد — PIN منفصل تمامًا عن الأدمن
  if (action === 'manageLogin') {
    return ContentService.createTextOutput(JSON.stringify(manageLogin_(e.parameter.pin)))
      .setMimeType(ContentService.MimeType.JSON);
  }
  ...
  // إشعار الأدمن: بيرجّع عدد الطلبات وآخر طلب بس (خفيف، للـ polling المتكرر)
  // من غير ما يرجّع كل بيانات الطلبات. للأدمن فقط.
  if (action === 'ordersMeta') {
    if (!isAdmin) return denyAdmin_();
    const rows = sheet ? sheet.getDataRange().getValues() : [];
    // عدد الطلبات الفريدة بالـ id (مش عدد الصفوف) — يطابق اللي بيتعرض في التطبيق
    const seen = {};
    ...
    let last = null;
    if (lastRow) {
      last = { id: String(lastRow[0] || ''), orderNo: String(lastRow[10] || ''), distName: String(lastRow[2] || '') };
    }
    return ContentService.createTextOutput(JSON.stringify({ count: count, last: last }))
      .setMimeType(ContentService.MimeType.JSON);
  }
  ...
  // ===== تبويب "إدارة" الجديد — كل الأكشنز هنا بتوكن منفصل (isManage) =====
  const isManage = isValidManageToken_(e.parameter.token);
  // نفس طلبات الباسورد، بس متاحة من تبويب الإدارة بتوكنه هو (مش توكن الأدمن).
  ...
  if (action === 'manageApprovePasswordReset' || action === 'manageRejectPasswordReset') {
    if (!isManage) return denyManage_();
    const ok = decidePwRequest_(e.parameter.id, action === 'manageApprovePasswordReset');
    return ContentService.createTextOutput(JSON.stringify({ ok: ok }))
      .setMimeType(ContentService.MimeType.JSON);
  }
  ...
  if (action === 'manageRegRequests') {
    if (!isManage) return denyManage_();
    return ContentService.createTextOutput(JSON.stringify(listRegRequests_()))
      .setMimeType(ContentService.MimeType.JSON);
  }
  if (action === 'manageApproveReg' || action === 'manageRejectReg') {
    if (!isManage) return denyManage_();
    const ok = decideRegRequest_(e.parameter.username, action === 'manageApproveReg');
    return ContentService.createTextOutput(JSON.stringify({ ok: ok }))
      .setMimeType(ContentService.MimeType.JSON);
  }
  // تصدير كل الأصناف (اسم/قطاع/تصنيف فرعي/محتوى/سعر/رصيد/حالة) — عشان تراجعهم
  // أو تعدّلهم في إكسيل وترفعهم تاني بنفس الشكل بالظبط.
  if (action === 'manageExportCatalog') {
    if (!isManage) return denyManage_();
    return ContentService.createTextOutput(JSON.stringify(exportCatalogRows_()))
      .setMimeType(ContentService.MimeType.JSON);
  }
  ...
  if (action === 'updateStatus') {
    ...
    for (let i = 1; i < rows.length; i++) {
      if (String(rows[i][0]) === String(id)) {
        ...
        // لما الحالة تبقى "تم تنفيذ الطلب" نبعت إشعار للموزع نفسه (مش للأدمن).
        // بيشتغل بس لو الموزع ليه مفتاح واتساب في شيت المستخدمين (عمود J).
        if (String(status).trim() === STATUS_DONE) {
          const distPhone = String(rows[i][4] || '');
          const key = getDistributorWaKey_(distPhone);
          if (key) {
            let msg = '✅ طلبك اتنفّذ بالكامل\n';
            msg += 'رقم الطلب: #' + String(rows[i][10] || id) + '\n';
        ...
        } else if (String(status).trim() === STATUS_RECEIVED) {
          // مرحلة وسطى — الطلب استُلم وجاري تجهيزه. إشعار اختياري بسيط للموزع.
          const distPhone = String(rows[i][4] || '');
          const key = getDistributorWaKey_(distPhone);
          if (key) {
            let msg = '📦 جاري تجهيز طلبك\n';
            msg += 'رقم الطلب: #' + String(rows[i][10] || id) + '\n';
            msg += 'الموزع: ' + String(rows[i][2] || '');
            queueWhatsApp_(msg, normalizePhone(distPhone), key);
            notified = true;
    ...
    // الحالة اتغيّرت (اتنفذت أو رجعت لمرحلة سابقة) — الحجز لازم يتحدّث فورًا
    syncReservedColumns_();
  ...
  if (action === 'deleteDelivered') {
    ...
    for (let i = rows.length - 1; i >= 1; i--) {
      if (rows[i][7] === STATUS_DONE) sheet.deleteRow(i + 1);
    }
